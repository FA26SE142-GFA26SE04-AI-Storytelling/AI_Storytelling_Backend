...
- **`deploy-core-api.yml` currently has no .NET SDK on the runner at all** — it only does `actions/checkout` + AWS/ECR/docker steps; the actual `dotnet build/publish` happens *inside* the multi-stage Dockerfile, not on the runner. Adding the §5.1 test job therefore needs its own `actions/setup-dotnet@v4` (`dotnet-version: '10.0.x'`, matching `dotnet-ci.yml`) — this was missing from the original design pass and would have made the test job fail immediately (`dotnet: command not found`) if implemented as first described.
- **Checked whether `tests/StoryPlatform.IntegrationTests` needs a live Postgres in CI (it would break the test gate if so):** inspected all 3 files. `AIStoryInputModelConfigurationTests.cs` calls `UseNpgsql("Host=localhost;...")` but only ever touches `context.Model` (EF Core's in-memory metadata built from `OnModelCreating`) — no connection is ever opened. `AIStoryGenerationClientTests.cs` constructs `HttpClient` with a fake/mock handler, no real network call. `Security/PasswordHasherTests.cs` is a pure unit test. **Conclusion: despite the project name, none of these tests need a real database or network access — `dotnet test StoryPlatform.sln` runs safely on a bare GitHub-hosted runner, no `services:` Postgres container needed.**
- **Verified `AutoDeploymentsEnabled` toggle is safe (no resource replacement):** queried the live `AWS::AppRunner::Service` CloudFormation resource schema (`aws cloudformation describe-type`) — `createOnlyProperties` is only `ServiceName`, `EncryptionConfiguration`, `Tags`. `SourceConfiguration` (which holds `AutoDeploymentsEnabled`) is not in that list, confirming the §5.5 change updates the existing service in place via `UpdateService`, no new ARN, no downtime from replacement.
...
New job `test`: checkout → **`actions/setup-dotnet@v4` (`dotnet-version: '10.0.x'`) — not currently present anywhere in this workflow, must be added** → `dotnet restore/build/test StoryPlatform.sln` (same commands as `dotnet-ci.yml`, for consistency). `build-and-push` gets `needs: test`. No AWS credentials needed for this job. Confirmed safe to run the full solution's test suite on a bare runner (see §3 finding on `IntegrationTests`) — no test infra (DB, mock server) needs to be stood up in CI.
